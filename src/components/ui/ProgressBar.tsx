...
export function ProgressBar({
  ...
  className = "",
  labelClassName = "text-[var(--tca-text-soft)]",
  percentClassName = "text-[var(--foreground)]",
}: ProgressBarProps) {
    ...
    <div className={`w-full ${className}`}>
      {(label || showPercent) && (
        <div className="mb-2 flex items-center justify-between gap-3">
          {label && <span className={`text-sm font-medium ${labelClassName}`}>{label}</span>}
          {showPercent && (
            <span className={`rounded-full border border-[var(--tca-border)] bg-white px-2.5 py-1 text-sm font-semibold shadow-[0_8px_18px_-18px_rgba(15,23,42,0.7)] ${percentClassName}`}>{percent}%</span>
          )}
        </div>
      )}
